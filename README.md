...
- `/stats leaderboard`, `/stats rating`, `/stats trend`, `/stats player`, `/stats history`, `/stats match_card`, `/stats streaks`, `/stats predict`, `/stats awards`, `/stats hall_of_fame`, `/stats quests`, `/stats elo_history`, `/stats confidence`, `/stats percentile`, `/stats mode_summary`, `/stats map_record`, `/stats form`, `/stats teammates`, `/stats teamleaderboard`, `/stats teamstats`, `/stats close_games`, `/stats comebacks` — view rankings, percentiles, map records, recent form, teammate history, career milestones, quests, and analytics.
- `/player profile`, `/player profile_set`, `/player search`, `/player aliases`, `/player best_mode`, `/player opponents`, `/player rivalry`, `/player recent_form`, `/player consistency`, `/player personal_bests`, `/player compare`, `/player myhistory` — view player records, configured aliases, and profiles. The comparison command uses Discord user selectors for both players.
...
- Match completion automatically posts an MVP, team score totals, and Elo changes. Leaderboards use embeds with previous/next page, minimum-games filtering, and refresh buttons. `/server help_menu` is an interactive paged list of every registered command.
...
Match cards use the included Gears 5 key art from the [official Gears of War website](https://www.gearsofwar.com/en-us/games/gears-5/) as a darkened background. The image is stored at `gears-background.jpg` (or under `assets/` when running from a source checkout). Rank artwork appears image-only in the compact `Rank` column beside each player’s team; the renderer makes a best-effort transparent crop of opaque screenshot-style rank images without changing the source files.
