...
The LAN dashboard includes all-mode and per-mode leaderboards, summary cards, player stat pages, player search, match detail pages, sortable JSON leaderboards (`?metric=rating|wins|games|kills|damage|score`), and JSON endpoints at `/api/health`, `/api/summary`, `/api/modes`, `/api/matches`, `/api/match/<id>`, `/api/player/<id>`, and `/api/leaderboard/<mode>`. Set `DASHBOARD_GUILD_ID` in `.env` when the database contains more than one Discord server and the dashboard should show only one server.
