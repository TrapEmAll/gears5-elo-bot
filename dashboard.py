...
def mode_page(mode: str):
    ...
    order = {"rating": "r.rating", "wins": "r.wins", "games": "r.games", "losses": "r.losses", "winrate": "CAST(r.wins AS REAL) / NULLIF(r.games, 0)"}.get(sort, "r.rating")
    search_clause = " AND (p.gamertag LIKE ? OR CAST(r.user_id AS TEXT) LIKE ?)" if search else ""
    search_params = (f"%{search}%", f"%{search}%") if search else ()
    rows = query(f"SELECT r.user_id, COALESCE(NULLIF(p.gamertag,''), CAST(r.user_id AS TEXT)) AS display_name, r.rating, r.wins, r.losses, r.games FROM ratings r LEFT JOIN player_profiles p ON p.guild_id=r.guild_id AND p.user_id=r.user_id WHERE {clause.replace('guild_id', 'r.guild_id')} AND r.mode=?{search_clause} ORDER BY {order} DESC, r.user_id LIMIT 50", params + (mode,) + search_params)
    ...
    return render_template_string(PAGE + """<div class=card><h2>{{ label }} leaderboard</h2><form><input name=q value="{{ search }}" placeholder="Search player or Discord ID"><label> Sort by: <select name=sort onchange="this.form.submit()"><option value=rating {% if sort == 'rating' %}selected{% endif %}>Elo</option><option value=wins {% if sort == 'wins' %}selected{% endif %}>Wins</option><option value=losses {% if sort == 'losses' %}selected{% endif %}>Losses</option><option value=winrate {% if sort == 'winrate' %}selected{% endif %}>Win rate</option><option value=games {% if sort == 'games' %}selected{% endif %}>Games</option></select></label><button>Search</button></form><div class=metrics>{% for key in ["kills", "deaths", "assists", "captures", "breaks", "damage", "score"] %}<div class=metric><small>Total {{ key }}</small><strong>{{ "{:,}".format(totals[key]) }}</strong></div>{% endfor %}</div>{% if rows %}<table><tr><th>#</th><th>Player</th><th>Discord ID</th><th>Elo</th><th>Record</th><th>Games</th></tr>{% for row in rows %}<tr><td>{{ loop.index }}</td><td><a href="/player/{{ row.user_id }}">{{ row.display_name }}</a></td><td>{{ row.user_id }}</td><td>{{ row.rating }}</td><td>{{ row.wins }}-{{ row.losses }}</td><td>{{ row.games }}</td></tr>{% endfor %}</table>{% else %}<p>No players matched this filter.</p>{% endif %}</div>""", rows=rows, totals=totals, label=MODES[mode], sort=sort, search=search, modes=MODES, refresh_seconds=REFRESH_SECONDS)
