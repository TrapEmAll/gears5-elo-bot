...
def home():
    ...
    clause, params = guild_clause(guild_id)
    leaderboards = [(mode, label, query(f"SELECT user_id, rating, wins, losses, games FROM ratings WHERE {clause} AND mode=? ORDER BY rating DESC LIMIT 10", params + (mode,))) for mode, label in selected_modes.items()]
...
def mode_page(mode: str):
    ...
    guild_id = requested_guild_id()
    clause, params = guild_clause(guild_id)
    rows = query(f"SELECT user_id, rating, wins, losses, games FROM ratings WHERE {clause} AND mode=? ORDER BY rating DESC LIMIT 50", params + (mode,))
    totals = query(f"SELECT COALESCE(SUM(kills),0) kills, COALESCE(SUM(deaths),0) deaths, COALESCE(SUM(damage),0) damage, COALESCE(SUM(score),0) score FROM match_player_stats WHERE {clause} AND mode=?", params + (mode,))[0]
    return render_template_string(PAGE + """<div class=card><h2>{{ label }} leaderboard</h2><div class=metrics>{% for key in ["kills", "deaths", "damage", "score"] %}<div class=metric><small>Total {{ key }}</small><strong>{{ "{:,}".format(totals[key]) }}</strong></div>{% endfor %}</div>{% if rows %}<table><tr><th>#</th><th>Player ID</th><th>Elo</th><th>Record</th><th>Games</th></tr>{% for row in rows %}<tr><td>{{ loop.index }}</td><td><a href="/player/{{ row.user_id }}">{{ row.user_id }}</a></td><td>{{ row.rating }}</td><td>{{ row.wins }}-{{ row.losses }}</td><td>{{ row.games }}</td></tr>{% endfor %}</table>{% else %}<p>No matches recorded for this mode.</p>{% endif %}</div>""", rows=rows, totals=totals, label=MODES[mode], modes=MODES, refresh_seconds=REFRESH_SECONDS)
...
def player_api(user_id: int):
    ...
    totals = query(f"SELECT COALESCE(SUM(kills),0) kills, COALESCE(SUM(deaths),0) deaths, COALESCE(SUM(assists),0) assists, COALESCE(SUM(damage),0) damage, COALESCE(SUM(score),0) score FROM match_player_stats WHERE {clause} AND user_id=?", params + (user_id,))[0]
...
def player_page(user_id: int):
    ...
    return render_template_string(PAGE + """<div class=card><h2>Player {{ display_name }}</h2><div class=metrics>{% for key in ["kills", "deaths", "assists", "damage", "score"] %}<div class=metric><small>Total {{ key }}</small><strong>{{ "{:,}".format(totals[key]) }}</strong></div>{% endfor %}</div><table><tr><th>Mode</th><th>Elo</th><th>Record</th><th>Games</th></tr>{% for row in rows %}<tr><td>{{ modes.get(row.mode, row.mode) }}</td><td>{{ row.rating }}</td><td>{{ row.wins }}-{{ row.losses }}</td><td>{{ row.games }}</td></tr>{% endfor %}</table></div><div class=card><h2>Recent performance</h2><table><tr><th>Match</th><th>Mode</th><th>Map</th><th>K/D</th><th>Assists</th><th>Damage</th><th>Score</th><th>Elo</th></tr>{% for row in history %}<tr><td>#{{ row.id }}</td><td>{{ modes.get(row.mode, row.mode) }}</td><td>{{ row.map_name }}</td><td>{{ row.kills }}/{{ row.deaths }}</td><td>{{ row.assists }}</td><td>{{ row.damage }}</td><td>{{ row.score }}</td><td>{{ "%+d"|format(row.rating_delta) }}</td></tr>{% endfor %}</table></div>""", display_name=display_name, totals=totals, rows=rows, history=history, modes=MODES, refresh_seconds=REFRESH_SECONDS)
